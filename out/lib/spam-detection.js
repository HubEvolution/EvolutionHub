...
exports.getSpamStats = getSpamStats;
/**
 ...
 */
function checkKeywords(content, customKeywords) {
...
function checkLinks(content) {
    ...
    const domains = urlMatches
        ...
        .filter(Boolean);
    const hasBlacklistedDomains = domains.some((domain) => BLACKLISTED_DOMAINS.some((pattern) => pattern.test(domain)));
 ...
 */
function checkCapsLock(content) {
 ...
 */
function determinePrimaryCategory(reasons) {
    const reasonText = reasons.join(' ').toLowerCase();
    if (reasonText.includes('keyword'))
        return 'keyword';
    if (reasonText.includes('link') || reasonText.includes('url'))
        return 'link';
    if (reasonText.includes('pattern') || reasonText.includes('caps'))
        return 'pattern';
    if (reasonText.includes('repetition') || reasonText.includes('repeated'))
        return 'repetition';
    if (reasonText.includes('length'))
        return 'length';
