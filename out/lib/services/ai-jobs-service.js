...
class AiJobsService extends base_service_1.AbstractBaseService {
    constructor(deps, env) {
        super(deps);
        this.env = env;
        this.log = logger_factory_1.loggerFactory.createLogger('ai-jobs-service');
    }
    creditsKey(userId) {
    ...
    async decrementCreditsBalance(userId) {
        const kv = this.env.KV_AI_ENHANCER;
        if (!kv)
            return 0;
        const key = this.creditsKey(userId);
    ...
    async incrementMonthlyUsage(ownerType, ownerId, limit, ym) {
        const kv = this.env.KV_AI_ENHANCER;
        if (!kv)
            return { used: 0, limit, resetAt: null };
        const useV2 = this.env.USAGE_KV_V2 === '1';
        if (useV2) {
            const res = await (0, usage_1.incrementMonthlyNoTtl)(kv, 'ai', ownerType, ownerId, limit);
    ...
    async createJob(params) {
        ...
        if (file.size > ai_image_1.MAX_UPLOAD_BYTES) {
            throw new Error(`File too large. Max ${Math.round(ai_image_1.MAX_UPLOAD_BYTES / (1024 * 1024))} MB`);
        }
        const fileBuffer = await file.arrayBuffer();
        const sniffed = (0, mime_1.detectImageMimeFromBytes)(fileBuffer);
        if (!sniffed || !ai_image_1.ALLOWED_CONTENT_TYPES.includes(sniffed)) {
        ...
        const monthlyLimit = typeof params.monthlyLimitOverride === 'number'
        ...
        if (monthly.used >= monthly.limit) {
            ...
            if (ownerType === 'user') {
                ...
                if (credits > 0) {
                    ...
                    const errM = new Error(`Monthly quota exceeded. Used ${monthly.used}/${monthly.limit}`);
        ...
        if (currentUsage.used >= currentUsage.limit) {
            ...
            const err = new Error(`Quota exceeded. Used ${currentUsage.used}/${currentUsage.limit}` +
        ...
        if (!bucket)
            throw new Error('R2_AI_IMAGES bucket not configured');
        ...
        await this.safeDbOperation(async () => {
            ...
            input_r2_key, input_content_type, input_size, created_at, updated_at
                ...
                .bind(id, ownerType === 'user' ? userId || ownerId : null, ownerType, ownerId, 'replicate', model.slug, inputKey, file.type, file.size)
                .run();
        });
    ...
    async getAndProcessIfNeeded(params) {
        const { id, ownerType, ownerId, requestOrigin } = params;
        const row = await this.safeDbOperation(async () => {
            return this.db
                .prepare(`SELECT id, user_id, owner_type, owner_ref, provider, model, status, provider_job_id,
        ...
        if (row.status === 'processing') {
            try {
                ...
                const resultKey = `${ai_image_1.AI_R2_PREFIX}/results/${ownerType}/${ownerId}/${Date.now()}.${resultExt}`;
                const bucket = this.env.R2_AI_IMAGES;
                if (!bucket)
                    throw new Error('R2_AI_IMAGES bucket not configured');
                ...
                await this.safeDbOperation(async () => {
                    await this.db
                        .prepare(`UPDATE ai_jobs SET status = 'succeeded', output_r2_key = ?, error_message = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
                ...
                const monthlyLimit2 = typeof params.monthlyLimitOverride === 'number'
                    ? params.monthlyLimitOverride
                    : Number.POSITIVE_INFINITY;
                ...
                await this.safeDbOperation(async () => {
                    await this.db
                        .prepare(`UPDATE ai_jobs SET status = 'failed', error_message = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
                        .bind(message, row.id)
                        .run();
                });
                row.status = 'failed';
    ...
    async cancelJob(params) {
        ...
        const row = await this.safeDbOperation(async () => {
            return this.db
                .prepare(`SELECT id, owner_type, owner_ref, status, input_r2_key, output_r2_key, model, provider, error_message, created_at, updated_at
           FROM ai_jobs WHERE id = ?`)
                .bind(id)
                .first();
        });
    ...
    async runReplicate(model, input) {
        ...
        if (!res.ok) {
            ...
            const err = (0, provider_error_1.buildProviderError)(status, 'replicate', text);
