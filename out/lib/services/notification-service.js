...
class NotificationService {
    ...
    async listNotifications(userId, filters = {}) {
        ...
        const now = Math.floor(Date.now() / 1000);
        whereConditions.push((0, drizzle_orm_1.or)((0, drizzle_orm_1.sql) `${schema_1.notifications.expiresAt} IS NULL`, (0, drizzle_orm_1.gte)(schema_1.notifications.expiresAt, now)));
        const baseWhere = (0, drizzle_orm_1.and)(...whereConditions);
        // Get total count
        const totalQuery = this.db.select({ count: (0, drizzle_orm_1.count)() }).from(schema_1.notifications);
        ...
        const unreadResult = await this.db
            ...
            .from(schema_1.notifications)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.notifications.userId, userId), (0, drizzle_orm_1.eq)(schema_1.notifications.isRead, false), (0, drizzle_orm_1.or)((0, drizzle_orm_1.sql) `${schema_1.notifications.expiresAt} IS NULL`, (0, drizzle_orm_1.gte)(schema_1.notifications.expiresAt, now))));
    ...
    async cleanupExpiredNotifications() {
        const now = Math.floor(Date.now() / 1000);
        const whereExpr = (0, drizzle_orm_1.and)((0, drizzle_orm_1.sql) `${schema_1.notifications.expiresAt} IS NOT NULL`, (0, drizzle_orm_1.lte)(schema_1.notifications.expiresAt, now));
        const toDelete = await this.db.select({ count: (0, drizzle_orm_1.count)() }).from(schema_1.notifications).where(whereExpr);
    ...
    async updateNotificationSettings(userId, request) {
        ...
        const existing = await this.db
            .select()
            .from(schema_1.notificationSettings)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.notificationSettings.userId, userId), (0, drizzle_orm_1.eq)(schema_1.notificationSettings.type, request.type), (0, drizzle_orm_1.eq)(schema_1.notificationSettings.channel, request.channel)))
            .limit(1);
    ...
    async initializeDefaultSettings(userId) {
        ...
        const defaultSettings = defaultTypes.flatMap((type) => ['in_app', 'email'].map((channel) => ({
            userId,
            type,
            channel: channel,
    ...
    async getEmailTemplateByName(name, locale = 'de') {
        const result = await this.db
            .select()
            .from(schema_1.emailTemplates)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.emailTemplates.name, name), (0, drizzle_orm_1.eq)(schema_1.emailTemplates.locale, locale), (0, drizzle_orm_1.eq)(schema_1.emailTemplates.isActive, true)))
            .limit(1);
        if (result.length === 0)
            return null;
        const r = result[0];
    ...
    async getUserNotificationSettingsFromEmail(email) {
        const userResult = await this.db.select().from(schema_1.users).where((0, drizzle_orm_1.eq)(schema_1.users.email, email)).limit(1);
    ...
    async getEmailQueueItemById(emailId) {
        ...
        return {
            ...
            scheduledFor: Math.floor((r.scheduledFor.getTime?.() ?? new Date(r.scheduledFor).getTime()) / 1000),
    ...
    async getPendingEmails(limit = 10) {
        ...
        const results = await this.db
            .select()
            .from(schema_1.emailQueue)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.emailQueue.status, 'pending'), (0, drizzle_orm_1.lte)(schema_1.emailQueue.scheduledFor, new Date(now * 1000))))
        ...
        return results.map((e) => ({
            ...
            scheduledFor: Math.floor((e.scheduledFor.getTime?.() ?? new Date(e.scheduledFor).getTime()) / 1000),
    ...
    async getNotificationStats(userId) {
        const baseWhere = userId ? (0, drizzle_orm_1.eq)(schema_1.notifications.userId, userId) : undefined;
        ...
        const stats = await statsQuery.groupBy(schema_1.notifications.type, schema_1.notifications.priority, schema_1.notifications.isRead);
        const result = {
            total: 0,
            unread: 0,
            byType: {
    ...
    async processEmailQueue() {
        ...
        await this.db
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.emailQueue.status, 'pending'), (0, drizzle_orm_1.lte)(schema_1.emailQueue.createdAt, new Date((now - maxAge) * 1000))));
    }
}
