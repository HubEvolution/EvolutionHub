...
async function getReferralStats(db, userId) {
  ...
  const creditsRow = await client
    .select({
      totalCredits: (0,
      drizzle_orm_1.sql)`COALESCE(SUM(${schema_1.referralEvents.creditsAwarded}), 0)`,
    })
    .from(schema_1.referralEvents)
    .where((0, drizzle_orm_1.eq)(schema_1.referralEvents.ownerUserId, userId))
    .limit(1);
...
async function getRecentReferralEvents(db, userId, limit = 5) {
  ...
  const rows = await client
    ...
    .where((0, drizzle_orm_1.eq)(schema_1.referralEvents.ownerUserId, userId))
    .orderBy(
      (0, drizzle_orm_1.desc)(schema_1.referralEvents.occurredAt),
      (0, drizzle_orm_1.desc)(schema_1.referralEvents.createdAt)
    )
    .limit(limit);
  return rows.map((row) => ({
    id: row.id,
