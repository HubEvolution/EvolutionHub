...
class PerformanceService {
    ...
    async getPaginatedComments(entityId, options, userId) {
        ...
        try {
            ...
            const { comments: commentData, total } = await this.fetchCommentsWithPagination(entityId, validatedOptions, userId);
            // Baue Kommentar-Tree mit Replies
            const commentsWithReplies = await this.buildCommentTree(commentData, validatedOptions, userId);
            const result = {
    ...
    async searchComments(options) {
        ...
        try {
            ...
            const commentsWithReplies = await this.buildCommentTree(searchResults, options.pagination || { page: 1, limit: 20 }, options.filters?.authorId?.[0]);
    ...
    async fetchCommentsWithPagination(entityId, options, _userId) {
        ...
        const query = this.db
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.comments.entityId, entityId), (0, drizzle_orm_1.eq)(schema_1.comments.status, 'approved'), options.includeReplies ? undefined : (0, drizzle_orm_1.eq)(schema_1.comments.parentId, null)))
    ...
    async performCommentSearch(options) {
        ...
        // Text-Suche
        whereConditions.push((0, drizzle_orm_1.sql) `${schema_1.comments.content} LIKE ${'%' + query + '%'}`);
        // Status-Filter
        if (filters?.status?.length) {
            whereConditions.push((0, drizzle_orm_1.or)(...filters.status.map((status) => (0, drizzle_orm_1.eq)(schema_1.comments.status, status))));
        }
        else {
        ...
        if (filters?.authorId?.length) {
            whereConditions.push((0, drizzle_orm_1.or)(...filters.authorId.map((id) => (0, drizzle_orm_1.eq)(schema_1.comments.authorId, id))));
        }
        // Datum-Filter
        if (filters?.dateFrom) {
    ...
    validatePaginationOptions(options) {
        return {
            ...
            limit: Math.min(this.config.pagination.maxLimit, Math.max(1, options.limit || this.config.pagination.defaultLimit)),
            sortBy: options.sortBy || 'createdAt',
            sortOrder: options.sortOrder || 'desc',
            includeReplies: options.includeReplies || false,
            maxDepth: Math.min(this.config.pagination.maxDepth, options.maxDepth || this.config.pagination.maxDepth),
        };
    }
    /**
