...
class CommentService {
    ...
    async updateComment(commentId, request, userId, csrfToken) {
        ...
        await this.db
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.comments.id, commentId), (0, drizzle_orm_1.eq)(schema_1.comments.authorId, userId)));
        // Create audit log for comment update
        await this.createAuditLog(commentId, userId, 'update', {
    ...
    async deleteComment(commentId, userId, csrfToken) {
        ...
        await this.createAuditLog(commentId, userId, 'delete', {
    ...
    async getCommentById(commentId) {
        const result = await this.db
          ...
          AND ${schema_1.commentReports.status} IN ('pending', 'reviewed')
            ...
            .leftJoin(schema_1.commentReports, (0, drizzle_orm_1.eq)(schema_1.commentReports.commentId, schema_1.comments.id))
    ...
    async listComments(filters = {}) {
        const { status, entityType, entityId, authorId, limit = 20, offset = 0, includeReplies = true, } = filters;
        const whereConditions = [];
        if (status) {
            whereConditions.push((0, drizzle_orm_1.eq)(schema_1.comments.status, status));
        }
        if (entityType && entityId) {
            whereConditions.push((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.comments.entityType, entityType), (0, drizzle_orm_1.eq)(schema_1.comments.entityId, entityId)));
        }
        if (authorId) {
        ...
        const baseWhere = whereConditions.length > 0 ? (0, drizzle_orm_1.and)(...whereConditions) : undefined;
        // Get total count
        const totalResult = await this.db.select({ count: (0, drizzle_orm_1.count)() }).from(schema_1.comments).where(baseWhere);
    ...
    async getCommentReplies(parentId) {
        const replyResults = await this.db
          ...
          AND ${schema_1.commentReports.status} IN ('pending', 'reviewed')
            ...
            .from(schema_1.comments)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.comments.parentId, parentId), (0, drizzle_orm_1.eq)(schema_1.comments.status, 'approved')))
    ...
    async moderateComment(commentId, request, moderatorId) {
        ...
        await this.createAuditLog(commentId, moderatorId, 'moderate', {
            status: currentComment.status,
        }, {
            status: newStatus,
        }, request.reason, {
    ...
    async reportComment(commentId, request, reporterId) {
        ...
        await this.createAuditLog(commentId, reporterId, 'report', undefined, undefined, `${request.reason}: ${request.description || 'No description'}`, {
            reporterEmail: request.reporterEmail,
    ...
    async getCommentReportCount(commentId) {
        const result = await this.db
            ...
            .from(schema_1.commentReports)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.commentReports.commentId, commentId), (0, drizzle_orm_1.eq)(schema_1.commentReports.status, 'pending')));
    ...
    async createAuditLog(commentId, userId, action, oldValues, newValues, reason, metadata) {
        const now = Math.floor(Date.now() / 1000);
        const ipAddr = metadata && typeof metadata.ipAddress === 'string'
            ? metadata.ipAddress
            : null;
        const ua = metadata && typeof metadata.userAgent === 'string'
    ...
    async getAuditLogs(filters = {}) {
        ...
        if (startDate) {
            whereConditions.push((0, drizzle_orm_1.sql) `${schema_1.commentAuditLogs.createdAt} >= ${startDate}`);
        }
        if (endDate) {
            whereConditions.push((0, drizzle_orm_1.sql) `${schema_1.commentAuditLogs.createdAt} <= ${endDate}`);
        }
        const baseWhere = whereConditions.length > 0 ? (0, drizzle_orm_1.and)(...whereConditions) : undefined;
        // Get total count
