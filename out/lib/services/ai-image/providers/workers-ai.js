...
exports.buildWorkersAiPayload = buildWorkersAiPayload;
function isRecord(v) {
    return typeof v === 'object' && v !== null;
}
function buildWorkersAiPayload(model, input) {
    ...
    // negative_prompt
    const providedNeg = isRecord(input) && typeof input.negative_prompt === 'string'
    ...
    else if (typeof model.defaultParams?.steps === 'number') {
        // Fallback if model.defaultParams used 'steps' instead of 'num_steps'
        payload.num_steps = model.defaultParams.steps;
    }
    return payload;
}
