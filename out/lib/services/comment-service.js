...
class CommentService {
    ...
    async isLegacyCommentsSchema() {
        if (this.schemaDetection)
            return this.schemaDetection.isLegacy;
        try {
            const info = await this.rawDb.prepare("PRAGMA table_info('comments')").all();
            // D1 returns rows with a 'name' field for column name
            const cols = new Set(Array.isArray(info?.results)
                ? info.results.map((r) => String(r.name))
                : []);
            const legacy = cols.has('postId') &&
                cols.has('approved') &&
                cols.has('createdAt') &&
    ...
    async invalidateEntityCache(entityType, entityId) {
        if (!this.kv || !entityType || !entityId)
            return;
        try {
    ...
    async createComment(request, userId, csrfToken) {
        ...
        if (spamCheck.isSpam) {
            throw new Error(`Comment rejected due to spam detection. Reasons: ${spamCheck.reasons.slice(0, 2).join(', ')}`);
        }
    ...
    async updateComment(commentId, request, userId, csrfToken) {
        ...
        const whereCond = isAdmin
            ...
            : (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.comments.id, commentId), (0, drizzle_orm_1.eq)(schema_1.comments.authorId, userId));
        await this.db
    ...
    async listComments(filters = {}) {
        ...
        const commentResults = await this.db
          ...
          SELECT image FROM users WHERE users.id = ${schema_1.comments.authorId} LIMIT 1
            ...
            .from(schema_1.comments)
            .where(baseWhere ? (0, drizzle_orm_1.and)(baseWhere, (0, drizzle_orm_1.isNull)(schema_1.comments.parentId)) : (0, drizzle_orm_1.isNull)(schema_1.comments.parentId))
        ...
        if (includeReplies && commentsWithReports.length > 0) {
            ...
            SELECT image FROM users WHERE users.id = ${schema_1.comments.authorId} LIMIT 1
                ...
                .from(schema_1.comments)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.inArray)(schema_1.comments.parentId, parentIds), (0, drizzle_orm_1.eq)(schema_1.comments.status, 'approved')))
    ...
    async moderateComment(commentId, request, moderatorId) {
        ...
        try {
            ...
            if (updatedComment && updatedComment.authorId && updatedComment.authorEmail) {
                ...
                const actionType = request.action === 'approve'
                    ? 'comment_approved'
