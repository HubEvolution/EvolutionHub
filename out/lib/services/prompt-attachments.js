...
function validateFiles(files) {
    ...
    for (const f of files) {
        ...
        if (f.size > prompt_enhancer_1.MAX_FILE_BYTES) {
            return { ok: false, reason: `File too large: ${f.name} (> ${prompt_enhancer_1.MAX_FILE_BYTES} bytes)` };
        }
    }
    return { ok: true };
}
...
async function uploadPdfFilesToProvider(openai, ctx, log) {
    ...
    for (const pdf of ctx.pdfs) {
        if (pdf.fileId || !pdf.file)
            continue;
        try {
            const created = await openai.files.create({ file: pdf.file, purpose: 'assistants' });
            pdf.fileId = created.id;
            if (log?.info)
                log.info('pdf_uploaded', { filename: pdf.filename, bytes: pdf.size });
        }
        catch (err) {
            if (log?.warn)
                log.warn('pdf_upload_failed', { filename: pdf.filename, error: err.message });
