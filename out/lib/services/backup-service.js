...
class BackupService {
    ...
    async createBackupJob(options, triggeredBy) {
        ...
        this.processBackupJob(jobId).catch((err) => (0, logger_1.log)('error', 'processBackupJob unhandled error', {
            jobId,
    ...
    async processBackupJob(jobId) {
        try {
            ...
            // Hole Job-Details
            const job = await this.db.select().from(schema_1.backupJobs).where((0, drizzle_orm_1.eq)(schema_1.backupJobs.id, jobId)).limit(1);
            if (!job[0]) {
    ...
    async getBackupJob(jobId) {
        const jobs = await this.db.select().from(schema_1.backupJobs).where((0, drizzle_orm_1.eq)(schema_1.backupJobs.id, jobId)).limit(1);
        const row = jobs[0];
        if (!row)
            return null;
        return {
            ...
            startedAt: row.startedAt != null
                ? (row.startedAt.getTime?.() ?? Number(row.startedAt))
                : undefined,
        ...
        try {
            ...
            const mapped = {
                ...
                startedAt: m.startedAt != null
                    ? (m.startedAt.getTime?.() ?? Number(m.startedAt))
                    : undefined,
                completedAt: m.completedAt != null
                    ? (m.completedAt.getTime?.() ?? Number(m.completedAt))
                    : undefined,
    ...
    async performCleanup() {
        ...
        await this.db
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.backupJobs.status, 'completed'), (0, drizzle_orm_1.lte)(schema_1.backupJobs.completedAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))));
        // Lösche alte Benachrichtigungen
        const cutoffSeconds = Math.floor(Date.now() / 1000) - 90 * 24 * 60 * 60;
        await this.db
            .delete(schema_1.notifications)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.notifications.isRead, true), (0, drizzle_orm_1.lte)(schema_1.notifications.readAt, cutoffSeconds)));
    ...
    async cleanupOldBackups(retentionDays = 30) {
        ...
        const candidates = await this.db
            ...
            .from(schema_1.backupJobs)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.backupJobs.status, 'completed'), (0, drizzle_orm_1.lte)(schema_1.backupJobs.completedAt, cutoff)));
    ...
    async restoreFromBackup(backupJobId, _targetTables) {
        ...
        (0, logger_1.log)('warn', 'WARNING: This operation is destructive and should be used with caution');
