...
class AiImageService {
    ...
    async runWorkersAi(model, input) {
        ...
        if (out instanceof Blob) {
            ...
            if (typeof orec.image === 'string') {
            ...
            else if (Array.isArray(orec.images) &&
                orec.images[0] instanceof Uint8Array) {
                ...
                if (typeof o === 'string') {
                    ...
                    o.images[0] instanceof Uint8Array) {
                    buf = o.images[0]
                        .buffer;
                }
            }
                ...
                if (typeof r === 'string') {
                    ...
                    r.images[0] instanceof Uint8Array) {
                    buf = r.images[0]
                        .buffer;
                }
            }
    ...
    async runWorkersAiWithRetry(model, input) {
        ...
        if ((first.arrayBuffer?.byteLength || 0) >= TINY_BYTES)
            return first;
        try {
        ...
        if ((second.arrayBuffer?.byteLength || 0) >= TINY_BYTES)
            return second;
    ...
    async callCustomAssistant(prompt, assistantId) {
        ...
        try {
            ...
            const content = first?.type === 'text' && typeof first?.text?.value === 'string' ? first.text.value : '';
            return { content };
        }
        catch (error) {
    ...
    async generate({ ownerType, ownerId, modelSlug, file, requestOrigin, scale, faceEnhance, assistantId, prompt, negativePrompt, strength, guidance, steps, limitOverride, monthlyLimitOverride, maxUpscaleOverride, allowFaceEnhanceOverride, }) {
        // Validate input
        ...
        if (assistantId) {
            ...
            try {
                ...
                if (typeof suggested.scale === 'number' &&
        ...
        const dailyLimit = typeof limitOverride === 'number'
            ? limitOverride
            : ownerType === 'user'
        ...
        if (currentUsage.used >= currentUsage.limit) {
            ...
            const msg = `Quota exceeded. Used ${currentUsage.used}/${currentUsage.limit}` +
        ...
        if (envName === 'development' || envName === 'testing') {
            ...
            if (model.provider === 'workers_ai') {
                ...
                if (envName === 'testing') {
                    ...
                    if (typeof strength === 'number')
                        strength = Math.min(0.6, Math.max(0.1, strength));
                    if (typeof guidance === 'number')
                        guidance = Math.min(9, Math.max(3, guidance));
            ...
            if (model.provider === 'workers_ai') {
                ...
                if (model.slug.startsWith('tencentarc/gfpgan') ||
    ...
    async fetchBinary(url) {
        ...
        if (!res.ok)
            throw new Error(`Failed to fetch output ${res.status} from ${url}`);
    ...
    async runReplicate(model, input) {
        ...
        if (usedPredictions) {
            if (!res.ok) {
                ...
                const { buildProviderError } = await Promise.resolve(`${'./' + 'provider-error'}`).then(s => require(s));
