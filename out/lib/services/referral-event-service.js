...
async function recordReferralSignup(db, options) {
  ...
  const profile = await client
    .select({
      userId: schema_1.referralProfiles.userId,
      code: schema_1.referralProfiles.referralCode,
    })
  ...
  const existing = await client
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.referralEvents.ownerUserId, ownerUserId),
        (0, drizzle_orm_1.eq)(schema_1.referralEvents.referredUserId, referredUserId)
      )
    )
    .limit(1);
