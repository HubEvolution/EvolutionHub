...
class DataExportService {
    ...
    async createExportJob(userId, options) {
        ...
        this.processExportJob(jobId).catch((err) => (0, logger_1.log)('error', 'processExportJob unhandled error', {
            jobId,
        ...
        if (options.type === 'comments' || options.type === 'full_export') {
            ...
            exportData.comments = commentsData.map((c) => ({
                ...
                updatedAt: c.updatedAt != null
                    ? (c.updatedAt.getTime?.() ?? Number(c.updatedAt))
                    : undefined,
        ...
        if (options.type === 'notifications' || options.type === 'full_export') {
            ...
            exportData.notifications = notificationsData.map((n) => ({
                ...
                readAt: n.readAt != null
                    ? (n.readAt.getTime?.() ?? Number(n.readAt))
                    : undefined,
            }));
        }
        return exportData;
    ...
    async getExportJob(jobId) {
        ...
        return {
            ...
            expiresAt: row.expiresAt
                ? (row.expiresAt.getTime?.() ?? Number(row.expiresAt))
                : undefined,
    ...
    async cleanupExpiredExports() {
        const expiredJobs = await this.db
            .select()
            .from(schema_1.dataExportJobs)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.dataExportJobs.status, 'completed'), (0, drizzle_orm_1.lte)(schema_1.dataExportJobs.expiresAt, new Date())));
    ...
    async processDeletionRequest(requestId, verificationToken) {
        const requests = await this.db
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.dataDeletionRequests.id, requestId), (0, drizzle_orm_1.eq)(schema_1.dataDeletionRequests.verificationToken, verificationToken), (0, drizzle_orm_1.eq)(schema_1.dataDeletionRequests.status, 'pending'), (0, drizzle_orm_1.gte)(schema_1.dataDeletionRequests.expiresAt, new Date())))
            .limit(1);
        ...
        this.performDataDeletion(request.userId, requestId).catch((err) => (0, logger_1.log)('error', 'performDataDeletion unhandled error', {
            requestId,
    ...
    async performDataDeletion(userId, requestId) {
        try {
            ...
            await this.db.delete(schema_1.comments).where((0, drizzle_orm_1.eq)(schema_1.comments.authorId, userId));
            await this.db.delete(schema_1.notifications).where((0, drizzle_orm_1.eq)(schema_1.notifications.userId, userId));
