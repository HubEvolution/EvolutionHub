...
async function stytchMagicLinkLoginOrCreate(context, req) {
    ...
    if (!res.ok) {
        ...
        const em = typeof err.error_message === 'string'
            ? err.error_message
            : typeof err.message === 'string'
                ? err.message
                : '';
        throw new StytchError(res.status, et, `Stytch login_or_create failed: ${res.status} ${et}${em ? ` - ${em}` : ''}`, typeof err.request_id === 'string' ? err.request_id : undefined);
    }
    return json;
}
        ...
        if (!res.ok) {
            ...
            const em = typeof err.error_message === 'string'
                ...
                : typeof err.message === 'string'
                    ? err.message
                    : '';
            throw new StytchError(res.status, et, `Stytch OAuth authenticate failed: ${res.status} ${et}${em ? ` - ${em}` : ''}`, typeof err.request_id === 'string' ? err.request_id : undefined);
        }
        return json;
    };
    try {
...
async function stytchMagicLinkAuthenticate(context, token, pkceCodeVerifier) {
    ...
    const doAttempt = async () => {
        const res = await fetchWithTimeout(url, {
            ...
            body: JSON.stringify(pkceCodeVerifier ? { token, pkce_code_verifier: pkceCodeVerifier } : { token }),
        }, 5000);
        const json = (await res.json());
        if (!res.ok) {
            const err = json;
            ...
            const em = typeof err.error_message === 'string'
                ...
                : typeof err.message === 'string'
                    ? err.message
                    : '';
            throw new StytchError(res.status, et, `Stytch authenticate failed: ${res.status} ${et}${em ? ` - ${em}` : ''}`, typeof err.request_id === 'string' ? err.request_id : undefined);
        }
        return json;
    };
    try {
