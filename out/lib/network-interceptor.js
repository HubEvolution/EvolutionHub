...
function installNetworkInterceptor() {
    ...
    const isStaticAsset = (url) => /\.(css|js|mjs|map|png|jpg|jpeg|gif|svg|webp|ico|woff2?|ttf|eot|txt|json|webmanifest)(\?|#|$)/i.test(url);
    ...
    window.fetch = async (...args) => {
        ...
        try {
            ...
            // Log response
            client_logger_1.clientLogger.info(`[NETWORK] ${method} ${safeUrl} → ${response.status} (${duration}ms, ${size}) [REDACTED]`, {
                source: 'network',
            ...
            const duration = Math.round(performance.now() - startTime);
            // Log error
            client_logger_1.clientLogger.error(`[NETWORK] ${method} ${safeUrl} FAILED (${duration}ms): ${error instanceof Error ? error.message : String(error)} [REDACTED]`, {
                source: 'network',
