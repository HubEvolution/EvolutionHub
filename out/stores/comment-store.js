...
exports.useCommentStore = (0, zustand_1.create)()((0, middleware_1.devtools)((set, get) => ({
    // Initial State
    comments: [],
    stats: null,
    currentUser: null,
    ...
    fetchComments: async (filters = {}, append = false) => {
        ...
        try {
            ...
            if (filters.authorId)
                queryParams.append('authorId', filters.authorId.toString());
            ...
            if (data.success) {
                ...
                set({
                    ...
                    stats: isAdmin && data.data.total !== undefined
                        ? {
                            total: data.data.total,
                            approved: data.data.comments.filter((c) => c.status === 'approved')
                                .length,
                            pending: data.data.comments.filter((c) => c.status === 'pending')
                                .length,
                            rejected: data.data.comments.filter((c) => c.status === 'rejected')
                                .length,
                            flagged: data.data.comments.filter((c) => c.status === 'flagged')
                                .length,
                            hidden: data.data.comments.filter((c) => c.status === 'hidden')
                                .length,
                        }
                        : null,
    ...
    createComment: async (data, csrfToken) => {
        ...
        const hasCommentById = (nodes, id) => {
            for (const n of nodes) {
                if (n.id === id)
                    return true;
                if (n.replies && n.replies.length && hasCommentById(n.replies, id))
                    return true;
            }
            return false;
        };
