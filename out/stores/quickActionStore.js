...
exports.selectQuickActionLoadingState = exports.selectQuickActionById = exports.selectQuickActions = void 0;
...
const useQuickActionStore = (0, zustand_1.create)((set, get) => ({
    ...
    updateActionStatus: (id, status) => set((state) => ({
        actions: state.actions.map((action) => action.id === id
            ? {
                ...action,
                status,
    ...
    executeAction: async (actionName) => {
        const action = get().actions.find((item) => item.action === actionName || item.id === actionName);
...
const selectQuickActionById = (id) => (state) => state.actions.find((action) => action.id === id) ?? null;
