...
(() => {
    ...
    function readVar(rootEl, name, fallback) {
        try {
            const root = (rootEl && rootEl) || document.documentElement;
    ...
    function findLandingScope(from) {
        if (!from)
            return document.querySelector('.landing-v2');
    ...
    function drawTechcellsFrame(ctx, canvas, scope, phase) {
        ...
        for (let row = -1; row < rows; row++) {
            ...
            for (let col = -1; col < cols; col++) {
                ...
                if (cx + w < 0 || cx - w > W || cy + h < 0 || cy - h > H)
                    continue;
    ...
    function boot() {
        const container = document.querySelector('[data-landing-bg]');
        if (!container)
            return;
        ...
        function drawFrame() {
            if (variant === 'techcells')
                drawTechcellsFrame(ctx, canvas, scope, t);
            else
                drawLatticeFrame(ctx, canvas, scope);
        }
        function loop() {
            drawFrame();
            t += 16;
