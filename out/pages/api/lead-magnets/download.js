...
exports.POST = (0, api_middleware_1.withApiMiddleware)(async (context) => {
    ...
    if (!requestData.leadMagnetId) {
        return (0, api_middleware_1.createApiError)('validation_error', 'Lead-Magnet-ID ist erforderlich');
    }
    if (!requestData.email || !validateEmail(requestData.email)) {
        return (0, api_middleware_1.createApiError)('validation_error', 'Gültige E-Mail-Adresse ist erforderlich');
    }
    ...
    if (!leadResult.success) {
        return (0, api_middleware_1.createApiError)('server_error', 'Fehler beim Speichern der Lead-Daten');
    }
...
exports.GET = (0, api_middleware_1.withApiMiddleware)(async (context) => {
    ...
    if (!leadMagnetId) {
        return (0, api_middleware_1.createApiError)('validation_error', 'Lead-Magnet-ID erforderlich');
    }
    ...
    if (shouldDownload) {
        ...
        if (source === 'r2') {
            ...
            if (!r2) {
                ...
                return (0, api_middleware_1.createApiError)('server_error', 'Datei-Service nicht verfügbar');
            }
            const obj = await r2.get(key);
            if (!obj) {
                // Audit: not_found
                try {
                    await locals.runtime.env.DB.prepare('INSERT INTO download_audit (id, created_at, ip, user_id, asset_key, status, bytes) VALUES (?, datetime("now"), ?, ?, ?, ?, ?)')
                        .bind(`dl_${Date.now()}_${Math.random().toString(36).slice(2)}`, getClientIP(request), null, key, 'not_found', 0)
                        .run();
                }
                catch (e) {
            ...
            try {
                await locals.runtime.env.DB.prepare('INSERT INTO download_audit (id, created_at, ip, user_id, asset_key, status, bytes) VALUES (?, datetime("now"), ?, ?, ?, ?, ?)')
                    .bind(`dl_${Date.now()}_${Math.random().toString(36).slice(2)}`, getClientIP(request), null, key, 'ok', size ?? 0)
                    .run();
            }
            catch (e) {
                // Audit-Insert fehlgeschlagen – bewusst ignoriert, um Download-Flow nicht zu stören
        ...
        return (0, api_middleware_1.createApiError)('server_error', 'Public downloads nicht über API verfügbar');
    }
...
exports.OPTIONS = (0, api_middleware_1.withApiMiddleware)(async () => {
