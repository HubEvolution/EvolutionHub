...
exports.GET = (0, api_middleware_1.withAuthApiMiddleware)(
  async (context) => {
    ...
    try {
      ...
      const job = await service.getBackupJob(jobId);
      if (!job) return (0, api_middleware_1.createApiError)('not_found', 'Backup job not found');
      return (0, api_middleware_1.createApiSuccess)({ job });
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'Failed to fetch backup job';
      return (0, api_middleware_1.createApiError)('server_error', msg);
    }
  },
  { logMetadata: { action: 'admin_backup_job_details' } }
);
