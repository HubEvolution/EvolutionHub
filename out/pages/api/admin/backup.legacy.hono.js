...
app.use('/*', async (c, next) => {
    const rid = (0, logger_1.generateRequestId)();
    c.set('requestId', rid);
    await next();
});
app.use('/*', (0, cors_1.cors)());
app.use('/*', (0, jwt_1.jwt)({ secret: process.env.JWT_SECRET }));
...
app.use('/*', async (c, next) => {
    try {
        const user = await (0, auth_helpers_1.requireAdmin)({
...
app.get('/jobs', async (c) => {
    try {
        ...
        const backupService = new backup_service_1.BackupService(db);
        ...
        (0, logger_1.log)('error', 'Error fetching backup jobs', {
...
app.get('/jobs/:id', async (c) => {
    try {
        ...
        (0, logger_1.log)('error', 'Error fetching backup job', {
            requestId: c.get('requestId'),
...
app.get('/jobs/:id/progress', async (c) => {
    try {
        ...
        (0, logger_1.log)('error', 'Error fetching backup progress', {
            requestId: c.get('requestId'),
...
app.post('/create', async (c) => {
    try {
        ...
        (0, logger_1.log)('error', 'Error creating backup job', {
            requestId: c.get('requestId'),
...
app.post('/schedule', async (c) => {
    try {
        ...
        (0, logger_1.log)('error', 'Error scheduling backup', {
            requestId: c.get('requestId'),
...
app.post('/maintenance/perform', async (c) => {
    try {
        ...
        const db = (0, d1_1.drizzle)(c.env.DB);
        const backupService = new backup_service_1.BackupService(db);
        const maintenanceId = await backupService.performMaintenance(body.type, body.description, adminId);
        return c.json({
            success: true,
            data: {
                maintenanceId,
        ...
        (0, logger_1.log)('error', 'Error starting maintenance', {
            requestId: c.get('requestId'),
...
app.get('/maintenance/jobs', async (c) => {
    try {
        ...
        (0, logger_1.log)('error', 'Error fetching maintenance jobs', {
            requestId: c.get('requestId'),
...
app.get('/maintenance/jobs/:id', async (c) => {
    try {
        ...
        (0, logger_1.log)('error', 'Error fetching maintenance job', {
            requestId: c.get('requestId'),
...
app.post('/cleanup', async (c) => {
    try {
        ...
        (0, logger_1.log)('error', 'Error during backup cleanup', {
            requestId: c.get('requestId'),
...
app.post('/verify/:id', async (c) => {
    try {
        ...
        (0, logger_1.log)('error', 'Error verifying backup', {
            requestId: c.get('requestId'),
...
app.get('/stats', async (c) => {
    try {
        ...
        (0, logger_1.log)('error', 'Error fetching backup statistics', {
            requestId: c.get('requestId'),
