  ...
  async (context) => {
    ...
    if (!userId && targetEmail) {
      try {
        const row = await db
          ...
          .bind(targetEmail)
          .first();
        if (!row?.id) return (0, api_middleware_1.createApiError)('not_found', 'User not found');
        userId = row.id;
      } catch {
    ...
    if (targetPlan === 'free') {
      ...
      if (!priceId || !stripeSecret) {
        return (0, api_middleware_1.createApiError)(
          'server_error',
          'Stripe not configured or price mapping missing'
        );
      }
      const stripe = new stripe_1.default(stripeSecret);
      ...
      try {
        if (
          ...
          await db
            ...
            .bind(
              ...
              typeof created.current_period_end === 'number' ? created.current_period_end : null
            )
            .run();
        }
      } catch {
        return (0, api_middleware_1.createApiError)(
          'server_error',
          'Failed to apply plan in Stripe'
        );
      }
    }
    try {
