...
exports.GET = (0, api_middleware_1.withAuthApiMiddleware)(async (context) => {
    ...
    if (!ip || !isValidIp(ip)) {
        ...
        return (0, api_middleware_1.createApiSuccess)({ city: '', country: '', display: '', ip: ip || '' });
    }
    try {
        // Provider 1: ipapi.co
        ...
        if (r1.ok) {
            ...
            const countryStr = typeof countryName === 'string'
                ? countryName
                : typeof countryAlt === 'string'
                    ? countryAlt
                    : '';
            const display = cityStr || countryStr ? `${cityStr ? cityStr + ', ' : ''}${countryStr}` : '';
            return (0, api_middleware_1.createApiSuccess)({ city: cityStr, country: countryStr, display, ip });
        }
        ...
        if (r2.ok) {
            ...
            const display = cityStr2 || countryStr2 ? `${cityStr2 ? cityStr2 + ', ' : ''}${countryStr2}` : '';
            return (0, api_middleware_1.createApiSuccess)({ city: cityStr2, country: countryStr2, display, ip });
        }
