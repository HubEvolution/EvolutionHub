...
async function upsertUser(db, email, desiredName, desiredUsername) {
    ...
    if (existing && existing.id) {
        ...
        await db
            .prepare('UPDATE users SET email_verified = 1, email_verified_at = COALESCE(email_verified_at, ?) WHERE id = ?')
    ...
    await db
        .prepare('INSERT INTO users (id, email, name, username, image, created_at, email_verified, email_verified_at) VALUES (?, ?, ?, ?, ?, ?, 1, ?)')
...
const getHandler = async (context) => {
    ...
    const devEnv = (context.locals?.runtime?.env
    ...
    if (!token) {
        ...
        return (0, response_helpers_1.createSecureRedirect)('/en/login?magic_error=MissingToken');
    }
    // Authenticate with Stytch (or dev bypass)
    let stytchEmail;
    const envRec = context.locals
        ?.runtime?.env;
    const bypassStr = typeof envRec?.STYTCH_BYPASS === 'string' ? envRec.STYTCH_BYPASS : undefined;
    ...
    if (isBypass) {
        ...
        try {
            ...
            const authRes = await (0, stytch_1.stytchMagicLinkAuthenticate)(context, token, pkceVerifier);
            const emails = authRes.user?.emails || [];
            stytchEmail = emails.find((e) => e.verified)?.email || emails[0]?.email;
            {
                const rid = authRes?.request_id;
            ...
            return (0, response_helpers_1.createSecureRedirect)('/en/login?magic_error=InvalidOrExpired');
        }
    }
    ...
    if (!db) {
        return (0, response_helpers_1.createSecureRedirect)('/en/login?magic_error=ServerConfig');
    }
    ...
    if (!email) {
        return (0, response_helpers_1.createSecureRedirect)('/en/login?magic_error=MissingEmail');
    }
    ...
    try {
        ...
        if (profCookie) {
            ...
            if (typeof data.name === 'string')
                desiredName = data.name;
            if (typeof data.username === 'string')
                desiredUsername = data.username;
        }
    }
    catch (_err) {
    ...
    let target = env?.AUTH_REDIRECT || '/dashboard';
    ...
    let effectiveLocale;
    try {
        const postAuthLocale = context.cookies.get('post_auth_locale')?.value;
        const prefLocale = context.cookies.get('pref_locale')?.value;
        effectiveLocale =
            postAuthLocale === 'de' || postAuthLocale === 'en'
                ...
                : prefLocale === 'de' || prefLocale === 'en'
                    ? prefLocale
                    : undefined;
        if (effectiveLocale && !(0, locale_path_1.isLocalizedPath)(target)) {
            target = (0, locale_path_1.localizePath)(effectiveLocale, target);
        }
    ...
    const hasLocale = (0, locale_path_1.isLocalizedPath)(target) || !!effectiveLocale;
    if (!hasLocale) {
    ...
    const response = (0, response_helpers_1.createSecureRedirect)(redirectTarget);
...
exports.GET = (0, api_middleware_1.withRedirectMiddleware)(getHandler);
