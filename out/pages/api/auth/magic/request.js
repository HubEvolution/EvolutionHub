        ...
        try {
            ...
            if (json && typeof json === 'object') {
                ...
                return {
                    ...
                    turnstileToken: typeof anyJson['cf-turnstile-response'] === 'string'
...
const handler = async (context) => {
    ...
    if (turnstileSecret) {
        if (!turnstileToken || typeof turnstileToken !== 'string' || turnstileToken.length < 10) {
            ...
            return (0, api_middleware_1.createApiError)('validation_error', 'Turnstile verification required');
        }
        ...
        try {
            ...
            if (!ok) {
                ...
                return (0, api_middleware_1.createApiError)('validation_error', 'Turnstile verification failed');
            }
        }
        catch {
            (0, security_logger_1.logMetricCounter)('turnstile_verify_unavailable', 1, { source: 'magic_request' });
            return (0, api_middleware_1.createApiError)('server_error', 'Turnstile verification unavailable');
        }
        (0, security_logger_1.logMetricCounter)('turnstile_verify_success', 1, { source: 'magic_request' });
    }
    const isDev = (cfEnv.ENVIRONMENT || cfEnv?.NODE_ENV) ===
        'development';
    ...
    const maybeProfile = {};
    if (isValidName(name))
        maybeProfile.name = name.trim();
    if (isValidUsername(username))
        maybeProfile.username = username;
    ...
    let pkceChallenge;
    const usePkce = cfEnv?.STYTCH_PKCE === '1' ||
        cfEnv?.STYTCH_PKCE === 'true';
    if (usePkce) {
    ...
    try {
        ...
        (0, security_logger_1.logMetricCounter)('auth_magic_request_success', 1, { source: 'magic_request' });
        if (devEnv) {
            console.log('[auth][magic][request] provider accepted', { ms: Date.now() - startedAt });
        }
    }
    catch (err) {
        (0, security_logger_1.logMetricCounter)('auth_magic_request_error', 1, { source: 'magic_request' });
        if (devEnv) {
            const e = err;
        ...
        if (err instanceof stytch_1.StytchError) {
            ...
            if (status >= 400 && status < 500) {
                return (0, api_middleware_1.createApiError)('validation_error', 'Provider rejected request');
            }
