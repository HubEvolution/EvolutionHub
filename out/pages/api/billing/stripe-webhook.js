...
const stripeWebhookLimiter = (0, rate_limiter_1.createRateLimiter)({
    maxRequests: 100,
    windowMs: 60 * 1000,
    name: 'stripeWebhook',
});
...
exports.POST = (0, api_middleware_1.withApiMiddleware)(async (context) => {
    ...
    const stripeSecret = typeof rawEnv.STRIPE_SECRET === 'string'
        ? rawEnv.STRIPE_SECRET
        : '';
    const webhookSecret = typeof rawEnv.STRIPE_WEBHOOK_SECRET === 'string'
        ? rawEnv.STRIPE_WEBHOOK_SECRET
        : '';
    if (!stripeSecret || !webhookSecret) {
        return (0, response_helpers_1.createSecureErrorResponse)('Stripe not configured', 500);
    }
    ...
    try {
        ...
        return (0, response_helpers_1.createSecureErrorResponse)('Invalid signature', 400);
    }
    const dbMaybe = rawEnv.DB;
    const kv = rawEnv.KV_AI_ENHANCER;
    if (!dbMaybe) {
        return (0, response_helpers_1.createSecureErrorResponse)('Database unavailable', 500);
    }
    const db = dbMaybe;
    ...
    async function upsertCustomer(userId, customerId) {
        await db
    ...
    const monthlyRaw = rawEnv.PRICING_TABLE;
    const annualRaw = rawEnv.PRICING_TABLE_ANNUAL;
    const monthlyMap = invert(parsePricingTable(monthlyRaw));
    const annualMap = invert(parsePricingTable(annualRaw));
    const priceMap = {};
    const isPlan = (v) => v === 'free' || v === 'pro' || v === 'premium' || v === 'enterprise';
    for (const [price, plan] of Object.entries({ ...monthlyMap, ...annualMap })) {
        if (typeof plan === 'string' && isPlan(plan))
            priceMap[price] = plan;
    }
    try {
        switch (event.type) {
            case 'checkout.session.completed': {
                ...
                if (session.mode === 'payment' && meta['purpose'] === 'credits' && kv) {
                    ...
                    if (userId && Number.isFinite(units) && units > 0) {
                        ...
                        await (0, usage_1.addCreditPackTenths)(kv, userId, packId, units * 10, typeof session.created === 'number' ? session.created * 1000 : Date.now());
                        // update legacy summary key for compatibility with existing UI
                        const totalTenths = await (0, usage_1.getCreditsBalanceTenths)(kv, userId);
        ...
        return (0, response_helpers_1.createSecureJsonResponse)({ received: true });
    }
    catch (err) {
        console.error('[stripe_webhook] error', err instanceof Error ? err.message : String(err));
        return (0, response_helpers_1.createSecureErrorResponse)('webhook_error', 500);
    }
}, {
