...
exports.GET = (0, api_middleware_1.withRedirectMiddleware)(async (context) => {
    ...
    const fallback = (reason) => {
        ...
        if (safeReturnTo)
            u.searchParams.set('return_to', safeReturnTo);
    ...
    try {
        ...
        const reqId = session?.lastResponse
            ?.requestId;
        if (reqId) {
            try {
                const secLogger = logger_factory_1.loggerFactory.createSecurityLogger();
                secLogger.logSecurityEvent('USER_EVENT', {
        ...
        if (session?.url) {
            return (0, response_helpers_1.createSecureRedirect)(session.url, 302, reqId ? { 'X-Stripe-Request-Id': reqId } : {});
        }
