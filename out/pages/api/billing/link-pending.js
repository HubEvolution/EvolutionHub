...
exports.GET = (0, api_middleware_1.withRedirectMiddleware)(async (context) => {
  ...
  const baseUrl =
    (typeof rawEnv.BASE_URL === 'string' ? rawEnv.BASE_URL : '') || `${url.protocol}//${url.host}`;
  if (!user) {
  ...
  if (!kv || !email) {
    return (0, response_helpers_1.createSecureRedirect)(
      `${baseUrl}/dashboard?billing=link-pending-missing`,
      302
    );
  }
  ...
  if (!raw) {
    return (0, response_helpers_1.createSecureRedirect)(
      `${baseUrl}/dashboard?billing=nopending`,
      302
    );
  }
  try {
    ...
    return (0, response_helpers_1.createSecureRedirect)(
      `${baseUrl}/dashboard?billing=link-error`,
      302
    );
  }
});
