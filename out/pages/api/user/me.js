...
exports.GET = (0, api_middleware_1.withAuthApiMiddleware)(async (context) => {
    ...
    rateLimiter: async (ctx) => {
        const { standardApiLimiter } = await Promise.resolve().then(() => require('@/lib/rate-limiter'));
        return standardApiLimiter(ctx);
    },
    // Standardisierte Unauthorized-Antwort (Tests erwarten error.type = 'auth_error')
    onUnauthorized: (_context) => (0, api_middleware_1.createApiError)('auth_error', 'Unauthorized'),
});
