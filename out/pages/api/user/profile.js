...
exports.POST = (0, api_middleware_1.withAuthApiMiddleware)(async (context) => {
    ...
    const locale = (0, accept_language_1.pickBestLanguage)(context.request.headers.get('accept-language') || null, 'de');
    const t = (0, i18n_1.getI18n)(locale);
    // Verbesserte Validierung mit Grenzen
    if (typeof name !== 'string' || name.length < 2 || name.length > 50) {
        return (0, api_middleware_1.createApiError)('validation_error', 'Name must be between 2 and 50 characters');
    }
    if (typeof username !== 'string' || username.length < 3 || username.length > 30) {
        return (0, api_middleware_1.createApiError)('validation_error', 'Username must be between 3 and 30 characters');
    }
    // Username-Format-Validierung mit RegEx
    const usernamePattern = /^[a-zA-Z0-9_]+$/;
    if (!usernamePattern.test(username)) {
        return (0, api_middleware_1.createApiError)('validation_error', 'Username may only contain letters, numbers and underscores');
    }
    ...
    if (!willChange) {
        if (role === 'user' && lastUpdate && nowMs - lastUpdate < cooldownMs) {
            ...
            const base = (0, api_middleware_1.createApiError)('rate_limit', msg, { retryAfterSeconds: retryAfterSec });
