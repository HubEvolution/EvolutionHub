...
exports.POST = (0, api_middleware_1.withApiMiddleware)(async (context) => {
    ...
    const environment = typeof rawEnv.ENVIRONMENT === 'string' ? rawEnv.ENVIRONMENT : undefined;
    const isDevEnv = environment === 'development' || import.meta.env.DEV === true;
    if (!isDevEnv) {
        (0, security_logger_1.logSecurityEvent)('PERMISSION_DENIED', {
        ...
        return (0, api_middleware_1.createApiError)('forbidden', 'Debug login not available');
    }
    // Optional additional protection: require a secret header if configured
    const expectedToken = typeof rawEnv.DEBUG_LOGIN_TOKEN === 'string'
        ? rawEnv.DEBUG_LOGIN_TOKEN
        : undefined;
    if (expectedToken) {
        const provided = context.request?.headers?.get('x-debug-token') ||
        ...
        if (!provided || provided !== expectedToken) {
            ...
            return (0, api_middleware_1.createApiError)('forbidden', 'Debug login token invalid');
        }
    }
    const db = env.DB;
    ...
    if (!user) {
        ...
        await db
            .prepare(`INSERT INTO ${userTableName} (id, email, name, username, created_at) VALUES (?, ?, ?, ?, ?)`)
    ...
    (0, security_logger_1.logUserEvent)(user.id, 'debug_login_created', {
        ipAddress: ip,
        debugUser: true,
    });
    ...
    onError: (context, error) => {
        ...
        (0, security_logger_1.logApiError)('/api/debug-login', {
            error: error instanceof Error ? error.message : String(error),
        }, { ipAddress: ip });
        return (0, api_middleware_1.createApiError)('server_error', 'Debug login failed');
    },
