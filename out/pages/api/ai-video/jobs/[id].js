...
exports.GET = (0, api_middleware_1.withApiMiddleware)(
  async (context) => {
    ...
    if (!metaRaw) return (0, api_middleware_1.createApiError)('not_found', 'Job not found');
    let meta;
    try {
      ...
      const type =
        statusRes.status === 401 || statusRes.status === 403
          ...
          : statusRes.status >= 400 && statusRes.status < 500
            ? 'validation_error'
            : 'server_error';
      return (0, api_middleware_1.createApiError)(type, 'Provider error', {
    ...
    if (pred.status === 'succeeded') {
      ...
      if (!outUrl && Array.isArray(out) && typeof out[0] === 'string') outUrl = out[0];
      if (!outUrl)
      ...
      if (!binRes.ok)
        return (0, api_middleware_1.createApiError)(
          'server_error',
          `Failed to fetch output (${binRes.status})`
        );
