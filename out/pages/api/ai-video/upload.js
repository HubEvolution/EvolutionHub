...
exports.POST = (0, api_middleware_1.withApiMiddleware)(async (context) => {
    ...
    try {
        ...
        if (typeof t === 'string' && (t === '720p' || t === '1080p'))
            tier = t;
    ...
    if (!file)
        return (0, api_middleware_1.createApiError)('validation_error', 'Videodatei (field "file") ist erforderlich');
    if (!tier)
        return (0, api_middleware_1.createApiError)('validation_error', 'Tier (field "tier") ist erforderlich');
    ...
    if (!ai_video_1.ALLOWED_VIDEO_CONTENT_TYPES.includes(ct)) {
        return (0, api_middleware_1.createApiError)('validation_error', `Unsupported content type: ${ct || 'unknown'}`);
    }
    const maxBytes = ai_video_1.MAX_UPLOAD_BYTES_TIER[tier];
    if (file.size > maxBytes) {
        return (0, api_middleware_1.createApiError)('validation_error', `File too large for ${tier}. Max ${(maxBytes / (1024 * 1024)) | 0} MB`);
    }
    ...
    if (durationSeconds > maxSec) {
        return (0, api_middleware_1.createApiError)('validation_error', `Video too long for ${tier}. Max ${maxSec}s`);
    }
