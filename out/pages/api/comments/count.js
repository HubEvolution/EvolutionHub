    ...
    try {
        ...
        if (q.get('debug') === '1') {
            try {
                ...
                const isLegacy = cols.includes('postId') &&
        ...
        if (!entityType || entityIds.length === 0) {
            return (0, api_middleware_1.createApiError)('validation_error', 'Missing entityType or entityId');
        }
        ...
        const isLegacy = cols.has('postId') &&
            cols.has('approved') &&
            cols.has('createdAt') &&
        ...
        if (entityIds.length === 1) {
            if (!isLegacy) {
                const res = await db
                    ...
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.comments.entityType, entityType), (0, drizzle_orm_1.eq)(schema_1.comments.entityId, entityIds[0]), (0, drizzle_orm_1.eq)(schema_1.comments.status, 'approved')));
        ...
        if (!isLegacy) {
            const res = await db
                ...
                .from(schema_1.comments)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.comments.entityType, entityType), (0, drizzle_orm_1.inArray)(schema_1.comments.entityId, entityIds), (0, drizzle_orm_1.eq)(schema_1.comments.status, 'approved')))
            ...
            const stmt = dbBinding
                .prepare(`SELECT postId as entityId, COUNT(*) as cnt FROM comments WHERE postId IN (${placeholders}) AND approved = 1 GROUP BY postId`)
                .bind(...entityIds);
            const res = await stmt.all();
            const map = Object.fromEntries(entityIds.map((id) => [id, 0]));
