...
app.post('/', async (c) => {
    try {
        ...
        const moderation = await commentService.moderateComment(commentId, moderationData, String(user.id));
...
const POST = async (context) => {
    try {
        ...
        const moderator = await (0, auth_helpers_1.requireModerator)({ request: context.request, env: { DB: env.DB } });
        ...
        const result = await service.moderateComment(commentId, moderationData, toModeratorId(moderator.id));
        ...
        if (message.startsWith('validation_error:')) {
            ...
            return (0, api_middleware_1.createApiError)('validation_error', (detail ?? '').trim() || message);
        }
