...
async function GET(context) {
    ...
    try {
        ...
        if (!bucket) {
            if (isDev)
                console.error(`[R2Proxy][${reqId}] R2_AI_IMAGES bucket not available`);
        ...
        if (category === 'results') {
            ...
            const currentOwnerId = currentOwnerType === 'user'
                ? locals.user.id
            ...
            if (!isOwner) {
                if (isDev)
                    console.warn(`[R2Proxy][${reqId}] forbidden for non-owner`);
        ...
        if (!object) {
            if (isDev)
                console.warn(`[R2Proxy][${reqId}] R2 object not found`, { key });
        ...
        if (isDev)
            console.error(`[R2Proxy][${reqId}] error`, error);
