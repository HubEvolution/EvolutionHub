...
async function loadTraffic24h() {
    try {
        ...
        const s = (Array.isArray(d.series) ? d.series : []).map((p) => typeof p.pageViews === 'number' ? p.pageViews : typeof p.visits === 'number' ? p.visits : 0);
...
function drawSparkline(svgId, values) {
    ...
    if (!svg)
        return;
    const width = svg.viewBox && svg.viewBox.baseVal ? svg.viewBox.baseVal.width : svg.clientWidth || 200;
    const height = svg.viewBox && svg.viewBox.baseVal ? svg.viewBox.baseVal.height : svg.clientHeight || 36;
...
function formatLastSeen(ms) {
    try {
        ...
        if (isFinite(sec)) {
            if (Math.abs(sec) < 60)
                rel = rtf.format(-sec, 'second');
            else if (Math.abs(sec) < 3600)
                rel = rtf.format(-Math.round(sec / 60), 'minute');
            else if (Math.abs(sec) < 86400)
                rel = rtf.format(-Math.round(sec / 3600), 'hour');
            else if (Math.abs(sec) < 2592000)
                rel = rtf.format(-Math.round(sec / 86400), 'day');
            else if (Math.abs(sec) < 31536000)
                rel = rtf.format(-Math.round(sec / 2592000), 'month');
            else
                rel = rtf.format(-Math.round(sec / 31536000), 'year');
        }
...
function setPaging(count, total) {
    ...
    if (cPage)
        cPage.textContent = `${from}–${to} von ${total}`;
    if (cPrev)
        cPrev.disabled = cOffset === 0;
    if (cNext)
        cNext.disabled = cOffset + count >= total;
}
async function loadComments() {
    try {
        ...
        if (cStatusVal && cStatusVal !== 'all')
            params.set('status', cStatusVal);
        ...
        const pageCount = d.pagination?.count ??
...
async function loadCreditsBalanceForUser() {
    ...
    try {
        ...
        const d = j.data;
        if (out)
            out.textContent = `Balance: ${d.credits} Credits (${d.tenths} tenth)`;
    }
    catch {
        if (err) {
            err.textContent = 'Laden fehlgeschlagen';
    ...
    try {
        ...
        if (stateEl) {
            const entriesHtml = Object.entries(state)
                ...
                const rows = entries
                    .map((e) => `<div class="text-white/50">${e.key} • count ${e.count} • reset ${e.resetAt ? new Date(e.resetAt).toLocaleTimeString() : '—'}</div>`)
                    .join('');
...
function start() {
    ...
    lookupBtn?.addEventListener('click', async (e) => {
        ...
        try {
            ...
            if (lrId)
                lrId.textContent = d.user.id;
            if (lrEmail)
                lrEmail.textContent = d.user.email;
            if (lrName)
                lrName.textContent = d.user.name || '';
            if (lrPlan)
                lrPlan.textContent = d.user.plan;
            if (lrCredits)
                lrCredits.textContent = String(d.credits);
            if (lrSub)
            ...
            if (lrLastIp)
                lrLastIp.textContent = d.lastIp || '—';
            if (lrLastSeen)
                lrLastSeen.textContent = d.lastSeenAt ? formatLastSeen(d.lastSeenAt) : '—';
            show(lookupResult);
            // Prefill grant email
            const ge = q('grant-email');
            if (ge && !ge.value)
                ge.value = d.user.email;
        }
        catch {
            if (lookupError) {
    ...
    lrIpGeoBtn?.addEventListener('click', async () => {
        try {
            ...
            if (lrIpGeoOut)
                lrIpGeoOut.textContent = finalText;
        }
        catch {
            if (lrIpGeoOut)
                lrIpGeoOut.textContent = '—';
            ...
            if (!res.ok || !isApiSuccess(json)) {
                if (grantError) {
                    grantError.textContent =
                        json?.error?.message || 'Gutschrift fehlgeschlagen';
                    show(grantError);
                }
                return;
            }
    ...
    const toggleCancelVisibility = () => {
        ...
        if (planCancelGroup) {
            if (p === 'free')
                planCancelGroup.classList.remove('hidden');
            else
                planCancelGroup.classList.add('hidden');
        }
    };
    ...
    planBtn?.addEventListener('click', async (e) => {
        ...
        try {
            ...
            if (intervalVal)
                body.interval = intervalVal;
            const prorationVal = planProration?.value || 'create_prorations';
            if (prorationVal)
                body.prorationBehavior = prorationVal;
            if (plan === 'free') {
                const immediate = !!(planCancelImmediate && planCancelImmediate.checked);
                if (immediate)
                    body.cancelImmediately = true;
                else
                    body.cancelAtPeriodEnd = true; // explicit for clarity
            }
            ...
            if (!res.ok || !isApiSuccess(json)) {
                if (planError) {
                    planError.textContent =
                        json?.error?.message || 'Plan setzen fehlgeschlagen';
                    show(planError);
                }
                return;
            }
    ...
    healthBtn?.addEventListener('click', async (e) => {
        ...
        try {
            ...
            if (healthStatus)
                healthStatus.textContent = `${res.status} • ${ms}ms`;
        }
        catch {
            if (healthStatus)
                healthStatus.textContent = 'Fehler';
        }
    });
