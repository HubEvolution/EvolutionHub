...
function useCompareInteractions(props) {
    const { containerRef, boxSize, setSliderPos, isHeld, setIsHeld, zoom, setZoom, pan, setPan, loupeEnabled, setLoupeEnabled, loupeSize, setLoupeSize, loupeFactor, setLoupeFactor, setLoupePos, setLoupeUiHint, compareVisible, } = props;
    ...
    const onHandleKeyDown = (0, react_1.useCallback)((e) => {
    ...
    const applyZoomAround = (0, react_1.useCallback)((direction, pivot) => {
    ...
    (0, react_1.useEffect)(() => {
        ...
        const onWheel = (ev) => {
            ...
            if (ev.shiftKey) {
                setLoupeSize((prev) => {
                    const next = Math.max(LOUPE_SIZE_MIN, Math.min(LOUPE_SIZE_MAX, Math.round(prev + (ev.deltaY < 0 ? LOUPE_SIZE_STEP : -LOUPE_SIZE_STEP))));
            ...
            if (ev.altKey) {
                setLoupeFactor((prev) => {
                    const next = Math.max(LOUPE_FACTOR_MIN, Math.min(LOUPE_FACTOR_MAX, Math.round((prev + (ev.deltaY < 0 ? LOUPE_FACTOR_STEP : -LOUPE_FACTOR_STEP)) * 10) /
                        10));
    ...
    const onMouseMoveLoupe = (0, react_1.useCallback)((e) => {
        const el = containerRef.current;
        if (!el)
            return;
        const rect = el.getBoundingClientRect();
    ...
    const onToggleLoupe = (0, react_1.useCallback)(() => setLoupeEnabled((v) => !v), [setLoupeEnabled]);
