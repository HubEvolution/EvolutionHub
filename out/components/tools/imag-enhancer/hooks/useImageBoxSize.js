...
function useImageBoxSize(deps = [], options = {}) {
    const { maxViewportHeightFraction = 0.75, minHeightPx = 200, fixedHeightPx, mode = 'auto', reservedTopPx = 0, reservedBottomPx = 0, safeAreaBottomPx = 0, } = options;
    ...
    const compute = (0, react_1.useCallback)(() => {
        if ((!imgRatio && !imgNatural) || !containerRef.current)
            return;
    ...
    return (0, react_1.useMemo)(() => ({ containerRef, boxSize, onResultImageLoad, onPreviewImageLoad }), [boxSize, onPreviewImageLoad, onResultImageLoad]);
}
