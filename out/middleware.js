...
exports.onRequest = (0, astro_middleware_1.defineMiddleware)(async (context, next) => {
  ...
  const isDevEnv = (cfEnv?.ENVIRONMENT || cfEnv?.NODE_ENV) === 'development';
  const isLoopback =
  ...
  if (!sessionWelcomeSeen) {
    ...
    if (m && m[1] === '1') {
      ...
      if (import.meta.env.DEV) {
        (0, logger_1.log)(
          'debug',
          '[Middleware] Fallback parsed session_welcome_seen from raw header',
          {
            requestId,
          }
        );
  ...
  if (setLocaleParam === 'de' || setLocaleParam === 'en') {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        (0, logger_1.log)('debug', '[Middleware] pref_locale cookie explicitly set', {
          requestId,
          targetLocale,
        });
      }
  ...
  if (!skipSplash && !sessionWelcomeSeen && !cookieLocale && !existingLocale) {
    try {
      ...
      if (import.meta.env.DEV) {
        (0, logger_1.log)(
          'debug',
          '[Middleware] Session splash gate -> set session_welcome_seen cookie',
          {
            requestId,
          }
        );
  ...
  if (isDePrefixed && !isApi && !isAsset && !isR2Proxy && !isAuthRoute(path)) {
    ...
    headers.set('Vary', 'Cookie, Accept-Language');
    if (import.meta.env.DEV) {
      (0, logger_1.log)('debug', '[Middleware] Normalize /de/* to canonical', {
        requestId,
        location,
      });
    }
  ...
  if (bot && !existingLocale && !isApi && !isAsset && !isR2Proxy) {
    ...
    if (botLocale === 'en') {
      ...
      headers.set('Vary', 'Cookie, Accept-Language');
      if (import.meta.env.DEV) {
        (0, logger_1.log)('debug', '[Middleware] Bot neutral -> redirect to EN', {
          requestId,
          location,
        });
      }
  ...
  if (!context.locals.runtime) {
    ...
    const sessionId =
      ...
      null;
    if (import.meta.env.DEV) {
      (0, logger_1.log)('debug', '[Middleware] Session ID from cookie', {
        requestId,
        present: !!sessionId,
      });
    }
    if (!sessionId) {
      ...
      try {
        const { session, user } = await (0, auth_v2_1.validateSession)(
  ...
  if (isAdminRoute(path)) {
    ...
    try {
      ...
      let role = 'user';
      if (db) {
        const row = await db.prepare('SELECT role FROM users WHERE id = ?').bind(u.id).first();
        role = row?.role || 'user';
      }
      if (role !== 'admin') {
        const targetLocale = existingLocale ?? preferredLocale;
  ...
  if (path.endsWith('.css')) {
    ...
    if (import.meta.env.DEV) {
      (0, logger_1.log)('debug', '[Middleware] Set Content-Type to text/css for CSS file', {
        requestId,
      });
    }
    ...
    if (import.meta.env.DEV) {
      (0, logger_1.log)(
        'debug',
        '[Middleware] Set Content-Type to application/javascript for JS file',
        {
          requestId,
        }
      );
    ...
    if (import.meta.env.DEV) {
      (0, logger_1.log)('debug', '[Middleware] Set Content-Type to image/svg+xml for SVG file', {
        requestId,
      });
    }
  }
