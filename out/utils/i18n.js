...
function getI18nArray(locale) {
  ...
  return (key) => {
    ...
    if (Array.isArray(fallbackVal)) return fallbackVal;
...
function selectPluralString(value, params) {
  ...
  if ('zero' in obj && count === 0 && typeof obj.zero === 'string') return obj.zero;
  if ('one' in obj && count === 1 && typeof obj.one === 'string') return obj.one;
  if ('other' in obj && typeof obj.other === 'string') return obj.other;
  return null;
}
