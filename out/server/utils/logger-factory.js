...
class ConsoleLogger {
    ...
    log(level, message, context) {
        if (!logging_1.LogUtils.isLogLevelEnabled(level))
            return;
        ...
        const method = consoleMethod === 'debug' ||
            consoleMethod === 'info' ||
            consoleMethod === 'warn' ||
            consoleMethod === 'error'
            ? consoleMethod
    ...
    createSecurityLogger(_config) {
        ...
        return {
            logSecurityEvent: (type, details, context) => {
                (0, logger_1.log)('info', 'SECURITY_EVENT', { type, details, ...(context || {}), timestamp: Date.now() });
            },
