...
test.describe('Kritische Benutzerabläufe', () => {
  test('Kompletter Anmeldevorgang', async ({ page, isMobile, navigateTo }) => {
    ...
    const logoutButton = page
      .locator('button[aria-label*="Logout"], button:has-text("Logout")')
      .first();
        ...
        try {
          ...
          if (navLinks) {
            ...
            await page.screenshot({
              path: 'test-results/mobile-menu-no-links.png',
              fullPage: true,
            });
          }
        } catch (error) {
  ...
  test('Login-Seite korrekt geladen', async ({ page, navigateTo }) => {
    ...
    const findElement = async (selectors: string[], elementName: string) => {
      ...
      console.warn(
        `⚠️ Kein ${elementName} gefunden. Folgende Selektoren wurden überprüft:`,
        selectors
      );
      return null;
    };
    ...
    const foundTexts = importantTexts.filter((text) => new RegExp(text, 'i').test(pageText));
