  ...
  test('should navigate to all main pages', async ({ page, isMobile, navigateTo }) => {
    ...
    for (let i = 0; i < linkCount; i++) {
      ...
      if (lowerHref.includes('login') || lowerHref.includes('register') || 
          lowerHref.includes('signin') || lowerHref.includes('signup')) {
      ...
      try {
        ...
        const safeFileName = `${i+1}-${linkText.replace(/[^a-z0-9]/gi, '-').toLowerCase()}`.replace(/-+/g, '-');
        ...
        const safeFileName = `error-${linkText.replace(/[^a-z0-9]/gi, '-').toLowerCase()}`.replace(/-+/g, '-');
