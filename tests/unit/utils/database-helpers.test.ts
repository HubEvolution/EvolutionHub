/**
 * Unit-Tests für die Datenbank-Helper (legacy implementation)
 * Konsolidiert aus test-suite-v2/src/unit/utils/database-helpers.test.ts
 */
...
} from '../../src/legacy/utils/database-helpers';
...
describe('Datenbank-Helper', () => {
  ...
  describe('Fixture-Laden', () => {
    it('sollte Test-Benutzer einfügen', async () => {
      ...
      expect(adminCall[1][5]).toBe('admin');
      expect(adminCall[1][6]).toBe(true);
    });
    ...
    it('sollte Newsletter-Abonnements einfügen', async () => {
      ...
      expect(newsletterCall[1][1]).toBe(true);
    });
  });
