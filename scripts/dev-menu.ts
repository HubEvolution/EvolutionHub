...
function displayMenu(options: MenuOption[], title: string) {
  console.clear();
    ...
    if (env === 'production') {
      ...
      rl.question(chalk.yellow('Eingabe: '), (answer) => {
        if (answer.trim() !== 'hub-evolution.com') {
          ...
          try {
            execSync(
              `npx --no-install wrangler d1 execute ${dbName} --env ${env} --file=${migrationPath}`,
              { stdio: 'inherit' }
            );
...
async function applyAllMigrations() {
  ...
  try {
    ...
    for (const migrationFile of migrationFiles) {
      ...
      try {
        execSync(
          `npx --no-install wrangler d1 execute evolution-hub-main-local --local --file=${migrationPath}`,
          { stdio: 'inherit' }
        );
...
async function recreateLocalDb() {
  ...
  try {
    ...
    try {
      execSync('npx --no-install wrangler d1 create evolution-hub-main-local', {
        stdio: 'inherit',
      });
    } catch (_error) {
