...
} from '../src/lib/validation/schemas/newsletter';
import {
  billingCreditsRequestSchema,
  billingCancelRequestSchema,
} from '../src/lib/validation/schemas/billing';
...
registry.register(
  ...
  newsletterSubscribeSchema.openapi({
    description:
      'Subscribe to newsletter with explicit consent and optional first name and source tag.',
  })
);
registry.register(
...
registry.register(
  ...
  billingCreditsRequestSchema.openapi({
    description: 'Purchase a credits pack and optionally link to a workspace and return URL.',
  })
);
registry.register(
  'BillingCancelRequest',
  billingCancelRequestSchema.openapi({
    description: 'Cancel a subscription by its Stripe subscriptionId.',
  })
);
registry.register(
  'TemplateSaveRequest',
...
registry.register(
  ...
  dashboardActionSchema.openapi({
    description:
      'Discriminated action to perform in the dashboard (create project/task, invite member, view docs).',
  })
);
registry.register(
  'InternalUserSyncRequest',
  internalUserSyncSchema.openapi({
    description: 'Minimal internal user payload used to sync id/email/name/image.',
  })
);
...
try {
  ...
  writeFileSync(
    resolve(outDir, 'zod-openapi-pilot.components.json'),
    JSON.stringify(document.components?.schemas ?? {}, null, 2)
  );
} catch (e) {
