...
export const GET = withAuthApiMiddleware(async (context: APIContext) => {
  ...
  // Stats per status (with same filters except status constraint)
  ...
  const toIso = (t?: number | null) => {
    ...
    const ms = t < 1_000_000_000_000 ? t * 1000 : t; // support epoch seconds
  ...
  try {
    const serverTiming = `query;desc=admin-comments;dur=${comments.length}`;
