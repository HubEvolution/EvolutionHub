...
import { stytchSuppressUser, StytchError } from '@/lib/stytch';
...
export const POST = withAuthApiMiddleware(
  async (context: APIContext) => {
    ...
    if (userRow.status === 'deleted') {
      return createApiError('validation_error', 'Cannot ban deleted user');
    }
    if (userRow.status === 'banned') {
      return createApiError('validation_error', 'User is already banned');
    }
    ...
    if (stytchUserId) {
      try {
        await stytchSuppressUser(context, stytchUserId);
      } catch (error) {
        if (error instanceof StytchError && error.status === 404) {
          // user already suppressed → ignore
        } else {
          await db
            .prepare('UPDATE users SET status = ?1, banned_at = ?2 WHERE id = ?3')
            .bind(previousStatus, previousBannedAt, userId)
            .run()
            .catch(() => undefined);
          ...
          const message =
            error instanceof StytchError ? error.message : 'Failed to suppress user in Stytch';
  ...
  },
  {
    ...
    logMetadata: { action: 'admin_user_ban' },
  }
);
