...
import { backupJobsQuerySchema, formatZodError } from '@/lib/validation';
...
export const GET = withAuthApiMiddleware(
  async (context: APIContext) => {
    ...
    const parsedQuery = backupJobsQuerySchema.safeParse({ limit: searchParams.get('limit') });
    ...
    try {
      ...
      const jobs = await service.getBackupJobs(limit);
      return createApiSuccess({ jobs });
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'Failed to fetch backup jobs';
      return createApiError('server_error', msg);
    }
  },
  { logMetadata: { action: 'admin_backup_jobs' } }
);
