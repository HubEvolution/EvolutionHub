...
export const POST = withAuthApiMiddleware(async (context: APIContext) => {
  ...
  try {
    body = (await context.request.json()) as BulkModerateBody;
  } catch {
  ...
  // Same-origin checks enabled by default; CSRF optional and not enforced here because UI does not send the token
