...
export const POST = async (context: APIContext) => {
  ...
  try {
    ...
    if (!isJson) {
      ...
      return createSecureJsonResponse({ error: true, message: 'Unsupported Media Type' }, 415);
    }
    ...
    if (!parse.success) {
      ...
      return createSecureJsonResponse({ error: true, message: 'Invalid email' }, 400);
    }
    ...
    const referer =
      typeof context?.request?.headers?.get === 'function'
        ...
        : '';
    const locale = referer.includes('/en/') ? 'en' : referer.includes('/de/') ? 'de' : 'en';

    if (!context.locals.runtime) {
      const errorMsg = 'Runtime environment is not available. Are you running in a Cloudflare environment?';
      console.error(errorMsg);
      return createSecureJsonResponse({ error: true, message: 'Server configuration error' }, 500);
    }
    ...
    const user = await db
      .prepare('SELECT id, email, name, username, email_verified FROM users WHERE email = ?')
      .bind(email)
      .first<UserRow>();

    // Immer generische Response nach außen, um Enumeration zu vermeiden
    const genericOk = () =>
      createSecureJsonResponse({ success: true, message: 'If an account exists, a verification email has been sent.' }, 200);

    if (!user) {
      // Nicht vorhandenen Benutzer nicht preisgeben
      logAuthFailure(context.clientAddress, { reason: 'resend_verification_non_existent_user' });
      return genericOk();
    }
...
if (!sendResult || !sendResult.success) {
  ...
  return genericOk();
    ...
    return createSecureJsonResponse({ error: true, message: 'Server error' }, 500);
  }
};

// 405 für nicht unterstützte Methoden
const methodNotAllowed = () =>
  createSecureJsonResponse({ error: true, message: 'Method Not Allowed' }, 405, { Allow: 'POST' });
