...
(function () {
  ...
  function drawTechcellsFrame(ctx, canvas, scope, phase) {
    ...
    ctx.fillRect(0, 0, W, H);
    var s = Math.max(8, Math.round(40 * dpr));
    ...
    var dashA = Math.max(10, Math.round(s * 1.0));
    var dashB = Math.max(6, Math.round(s * 0.7));
    var dashOffset = -((phase / 28) % (dashA + dashB));
    var alphaMain = parseFloat(readVar(scope, '--techcells-alpha-main', '0.065')) || 0.065;
    var alphaAccent = parseFloat(readVar(scope, '--techcells-alpha-accent', '0.11')) || 0.11;
    var accentProb = parseFloat(readVar(scope, '--techcells-accent-prob', '0.01')) || 0.01;
    var period = 3000,
      width = 0.18;
    function pulseFactor(row, col) {
      var v = Math.sin(row * 12.9898 + col * 78.233) * 43758.5453;
      var r = v - Math.floor(v);
      var local = (phase / period + r) % 1;
      var tri = 1 - Math.abs(local - 0.5) / width;
      return { f: Math.max(0, Math.min(1, tri)), warm: r < 0.5 };
    }
    function drawHex(cx, cy, row, col) {
      var pts = [];
      for (var i = 0; i < 6; i++) {
      ...
      ctx.closePath();
      ctx.stroke();
      var pf = pulseFactor(row, col);
      if (pf.f > 0) {
        var c = pf.warm ? gridAccentWarm : gridAccent;
        var pulseAlpha = Math.min(0.24, alphaAccent + 0.1) * pf.f;
        ctx.setLineDash([]);
        ctx.strokeStyle = 'rgba(' + c[0] + ',' + c[1] + ',' + c[2] + ',' + pulseAlpha + ')';
        ctx.beginPath();
        ctx.moveTo(pts[0].x, pts[0].y);
        for (var k = 1; k < 6; k++) ctx.lineTo(pts[k].x, pts[k].y);
        ctx.closePath();
        ctx.stroke();
      }
      if (Math.random() < accentProb) {
        var iedge = Math.floor(Math.random() * 6);
        var a = pts[iedge],
          b = pts[(iedge + 1) % 6];
        ctx.setLineDash([]);
        ctx.strokeStyle =
    ...
    for (var r = -1; r < rows; r++) {
      ...
      for (var c = -1; c < cols; c++) {
        var cx = c * hStep + (r % 2 ? hStep / 2 : 0) + 0.5;
        if (cx + w < 0 || cx - w > W || cy + h < 0 || cy - h > H) continue;
        drawHex(cx, cy, r, c);
      }
    }
    ctx.restore();
    var mask = ctx.createLinearGradient(0, 0, 0, Math.min(H * 0.45, 520 * dpr));
    var topMaskAlpha = Math.max(
      0,
      Math.min(1, parseFloat(readVar(scope, '--techcells-top-mask-alpha', '0.18')) || 0.18)
    );
