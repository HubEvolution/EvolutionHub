...
export async function findSessionById(
  db: D1Database,
  sessionId: string
): Promise<D1FirstResult<Session & { user: User }>> {
  return safeDbOperation(
    async () => {
      return await db
        .prepare(
...
export async function findActivitiesByUserId(
  ...
  return safeDbOperation(
    ...
    `
        ...
        .bind(userId, limit)
        .all<Activity & { user_name: string; user_image?: string }>();
