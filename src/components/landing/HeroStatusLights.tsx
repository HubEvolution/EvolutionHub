...
export default function HeroStatusLights({ labels, icons, refreshMs = 180000, iconsOnly = false }: Props) {
  const [status, setStatus] = useState<StatusMap>({ infra: null, image: null, prompt: null, voice: null, web: null });

  const endpoints = useMemo(() => {
    const prefix = typeof window !== 'undefined' && window.location.pathname.startsWith('/en/') ? '/en' : '';
    return {
      infra: '/api/health',
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        ...
        const toBoolWithKey = async (key: keyof typeof endpoints, r: PromiseSettledResult<Response>) => {
        ...
        if (mounted) setStatus({ infra: infraOk, image: imageOk, prompt: promptOk, voice: voiceOk, web: webOk });
      } catch {
        if (mounted) setStatus({ infra: false, image: false, prompt: false, voice: false, web: false });
      }
    };
  ...
  function ToolIcon({ name, titleText }: { name: 'infra' | 'image' | 'prompt' | 'voice' | 'web'; titleText?: string }) {
    ...
    if (custom && custom !== 'default') {
      ...
      if (known.includes('image') || known === 'imag' || known === 'photo' || known === 'gallery') name = 'image';
      else if (known.includes('prompt') || known === 'magic' || known === 'pencil' || known === 'edit') name = 'prompt';
      else if (known.includes('voice') || known === 'mic' || known === 'microphone' || known === 'audio') name = 'voice';
      else if (known.includes('web') || known.includes('scrape') || known === 'globe' || known === 'search') name = 'web';
      else if (known.includes('infra') || known.includes('server') || known.includes('cloud') || known === 'cpu') name = 'infra';
      else {
        return (
          <svg className={`${cls} text-slate-300`} width={52} height={52} viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true" data-icon={custom}>
            {titleText ? <title>{titleText}</title> : null}
            <circle cx="12" cy="12" r="9" strokeWidth="1.75" opacity="0.35" />
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.85" d="M8 12h8M12 8v8" />
          </svg>
        );
      }
    }
    if (name === 'image') {
      return (
        <svg className={`${cls} text-emerald-300`} width={52} height={52} viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
          {titleText ? <title>{titleText}</title> : null}
          <rect x="3" y="5" width="18" height="14" rx="2" strokeWidth="1.85" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.85" d="M7 14l3-3 3 2 3-3 4 4" />
    ...
    if (name === 'prompt') {
      return (
        <svg className={`${cls} text-cyan-300`} width={52} height={52} viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
          {titleText ? <title>{titleText}</title> : null}
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.85" d="M4 20l9-9" />
          <rect x="11.2" y="7.8" width="3.2" height="1.6" rx="0.5" transform="rotate(45 11.2 7.8)" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.6" d="M15.5 4.5v2M15.5 3.5v0M14.5 5.5h2" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.6" d="M19 8.5v2M18 9.5h2" />
        </svg>
      );
    }
    if (name === 'voice') {
      return (
        <svg className={`${cls} text-indigo-300`} width={52} height={52} viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
          {titleText ? <title>{titleText}</title> : null}
          <rect x="9" y="4" width="6" height="10" rx="3" strokeWidth="1.85" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.85" d="M6 11a6 6 0 0012 0" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.6" d="M4.8 9.5c-1.2 1.2-1.2 3.8 0 5" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.6" d="M19.2 9.5c1.2 1.2 1.2 3.8 0 5" />
        </svg>
      );
    }
    if (name === 'web') {
      return (
        <svg className={`${cls} text-amber-300`} width={52} height={52} viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
          {titleText ? <title>{titleText}</title> : null}
          <circle cx="12" cy="12" r="9" strokeWidth="1.85" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.6" d="M3 12h18" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.6" d="M12 3a14 14 0 0 1 0 18" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.6" d="M12 3a14 14 0 0 0 0 18" />
        </svg>
      );
    }
    // infra (servers)
    return (
      <svg className={`${cls} text-slate-300`} width={52} height={52} viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
        {titleText ? <title>{titleText}</title> : null}
        ...
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.6" d="M6 6h.01M6 12h.01M6 18h.01" />
      </svg>
    );
  }
    ...
    <div className="w-full">
      <div className="container mx-auto px-4 grid grid-cols-3 grid-rows-3 sm:grid-rows-1 sm:grid-cols-5 gap-1 sm:gap-6 md:gap-12 lg:gap-12">
        {items.map((it, idx) => {
          ...
          return (
            <div key={it.key} className={`relative group justify-self-center w-[84px] sm:w-[112px] md:w-[128px] lg:w-[144px] ${posClass}`}>
              {href ? (
                <a href={href} className="block focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/70 rounded-sm" aria-label={`Open ${it.label}`} title={it.label}>
                  <div
                    ...
                    style={{ clipPath: hexClip as any, WebkitClipPath: hexClip as any, aspectRatio: '2 / 1.732' }}
                  >
                    <span className="pointer-events-none absolute inset-0 hex-shape landing-holo-bg" aria-hidden="true" style={{ clipPath: hexClip as any, WebkitClipPath: hexClip as any }} />
                    <span
                      ...
                      style={{
                        ...
                        boxShadow: 'inset 0 0 28px rgba(0,0,0,0.34), inset 0 18px 36px rgba(0,0,0,0.18), inset 0 -18px 36px rgba(0,0,0,0.18)'
                      }}
                    />
                    <span className={`h-1.5 w-1.5 rounded-full ${colorClass(it.value)}`} />
                    <ToolIcon name={it.key} titleText={it.label} />
                    {!iconsOnly && (
                      <span className={`text-[12px] font-semibold tracking-wider status-label ${textClass(it.value)}`}>
                        {it.label}
                      </span>
                    )}
                  </div>
