...
function Card({ item, entering }: { item: TestimonialItem; entering?: boolean }) {
  return (
    <article className="relative rounded-2xl overflow-hidden p-6 bg-white/5 dark:bg-white/3 backdrop-blur-xl border border-white/10 hover:border-white/20 transition-shadow transform-gpu hover:-translate-y-0.5 landing-holo-panel h-full min-h-[180px] md:min-h-[200px]">
      <div className="absolute -top-10 -left-6 text-7xl select-none opacity-[0.12] pointer-events-none">“</div>
      <blockquote
        className={`text-base leading-relaxed text-gray-900 dark:text-gray-100 ${entering ? 'animate-child-1' : ''}`}
        style={{ display: '-webkit-box', WebkitBoxOrient: 'vertical', overflow: 'hidden', WebkitLineClamp: 5 }}
      >
        {item.quote}
      </blockquote>
      ...
      <div className="absolute inset-0 pointer-events-none rounded-2xl gradient-border" aria-hidden="true" />
    </article>
  );
}

export default function TestimonialsRotatingGrid({ items, intervalMs = 6500, className = '' }: Props) {
  const prefersReduced = useReducedMotion();
  // 5 visible indices for 2-1-2 layout
  const [visible, setVisible] = useState<number[]>(() => {
    const base = [0, 1, 2, 3, 4].filter((i) => i < items.length);
    while (base.length < 5 && base.length < Math.max(5, items.length)) base.push(base.length % Math.max(1, items.length));
  ...
  const [prevItems, setPrevItems] = useState<(TestimonialItem | null)[]>([null, null, null, null, null]);
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      setVisible((current) => {
        ...
        recentIndicesRef.current = [next, ...recentIndicesRef.current].slice(0, Math.min(8, items.length));
        // release after fade-in finishes
        const inMs = (typeof window !== 'undefined' && window.matchMedia && window.matchMedia('(min-width: 768px)').matches) ? 900 : 750;
        window.setTimeout(() => { setPrevItems((p) => { const c = p.slice(); c[slot] = null; return c; }); animatingRef.current = false; }, inMs);
        return updated;
      });
    };
  ...
  return (
    <div ref={rootRef} aria-live="polite" className={`flex flex-col gap-y-10 md:gap-y-16 ${className}`}
      onMouseEnter={() => (hoverRef.current = true)}
      onMouseLeave={() => (hoverRef.current = false)}>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
        {[0,1].map((slot) => (
          <div key={`row1-${slot}-${visible[slot]}`} className={`relative ${slot===0 ? 'md:-mt-4' : 'md:mt-4'} ${lastChangedRef.current === slot ? 'animate-fade-in' : ''}`}>
            {prevItems[slot] ? (
      ...
      <div className="flex justify-center">
        <div className={`relative ${lastChangedRef.current === 2 ? 'animate-fade-in' : ''} w-full sm:w-[min(32rem,100%)] md:-mt-2`}>
          {prevItems[2] ? (
            <div className="absolute inset-0 animate-fade-out">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
        {[3,4].map((slot) => (
          <div key={`row3-${slot}-${visible[slot]}`} className={`relative ${slot===3 ? 'md:mt-4' : 'md:-mt-4'} ${lastChangedRef.current === slot ? 'animate-fade-in' : ''}`}>
            {prevItems[slot] ? (
              <div className="absolute inset-0 animate-fade-out">
                <Card item={prevItems[slot]!} />
              </div>
