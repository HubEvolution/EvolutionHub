...
- R2 Proxies:
  - Avatars: `src/pages/r2/[...path].ts` (`R2_AVATARS`)
  - AI Images: `src/pages/r2-ai/[...path].ts` (`R2_AI_IMAGES`)
...
- Storage (R2)
  ...
  - Schlüssel (tatsächliches Schema):
    - `ai-enhancer/uploads/<ownerType>/<ownerId>/<timestamp>.<ext>`
    - `ai-enhancer/results/<ownerType>/<ownerId>/<timestamp>.<ext>`
...
Response‑Shapes (aktuell):

- POST `/api/ai-image/jobs` (202):
...
```json
{
  ...
  "data": {
    ...
    "input": { "key": "<r2-key>", "url": "<public-url>", "contentType": "image/png", "size": 12345 },
    "output": null,
    "error": null,
    "createdAt": "<iso>",
    "updatedAt": "<iso>",
...
```json
{
  ...
  "data": {
    "id": "<uuid>",
    "status": "processing|succeeded|failed|canceled|queued",
    "provider": "replicate",
    "model": "<replicate-slug>",
    "input": { "key": "<r2-key>", "url": "<public-url>", "contentType": "image/png", "size": 12345 },
    "output": { "key": "<r2-key>", "url": "<public-url>" } | null,
    "error": "<message>" | null,
    "createdAt": "<iso>",
    "updatedAt": "<iso>"
  }
...
- POST `/api/ai-image/jobs/[id]/cancel` (200): identisch zur Job‑Response, `status: "canceled"`.
...
- Ergebnis‑Auslieferung (implementiert)
  ...
  - Middleware‑Exklusion: `/r2-ai/*` wird in `src/middleware.ts` explizit von Locale‑Redirects und Gating ausgenommen.
  - Trailing‑Slash‑Redirects: GELÖST am 2025‑09‑01 via `trailingSlash: "ignore"` in `astro.config.mjs`. Upload‑URLs unter `/r2-ai/ai-enhancer/uploads/...` liefern direkt (keine 301/308). Middleware‑Exklusion für `/r2-ai/*` bleibt bestehen.
...
- [x] Trailing‑Slash‑Redirect‑Fix für `/r2-ai/ai-enhancer/uploads/...` (umgesetzt via `trailingSlash: "ignore"`)
...
## 8) Client‑UI (React Island)
 ...
 - Seiten: `src/pages/tools/imag-enhancer/app.astro` (DE), `src/pages/en/tools/imag-enhancer/app.astro` (EN)
 ...
 - Einbindung: Island wird in `app.astro` importiert (sobald erstellt).
...
## 10) Tests
 ...
 - Dev‑Server für Tests: Playwright/Vitest nutzen den Cloudflare Wrangler‑Dev‑Server unter `http://localhost:8787`. Tests lesen `TEST_BASE_URL` (Fallback `BASE_URL`); ist keine gesetzt, startet das Global‑Setup automatisch `npm run dev` (Wrangler).
...
- [x] Wrangler: `KV_AI_ENHANCER` Namespace‑IDs in `wrangler.toml` in allen Umgebungen ersetzen (dev/staging/prod)
- [x] Trailing‑Slash‑Redirect‑Fix für `/r2-ai/ai-enhancer/uploads/...` (umgesetzt via `trailingSlash: "ignore"`)
