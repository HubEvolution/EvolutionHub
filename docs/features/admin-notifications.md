...
- Processing loop:
  ...
  1. Failures increment `attempts`, persist `last_error`, and remain queued for retry.@src/lib/services/notification-service.ts#619-633
  1. Items pending >24h are automatically marked as failed by `processEmailQueue()`.@src/lib/services/notification-service.ts#692-707
...
1. Optional internal auth health `GET /api/health/auth` when `INTERNAL_HEALTH_TOKEN` is provided.@scripts/warmup.ts#90-109
1. Warmup targets (defined later in the file) fetch static pages, API endpoints, and SSE/Poll routes to prime caches.
