...
1. Seite öffnen: `/tools/imag-enhancer/app` (DE) oder `/en/tools/imag-enhancer/app` (EN)
1. Bild (PNG/JPG/WEBP) hochladen, Modell wählen, optional Scale/FaceEnhance (je nach Fähigkeit), „Enhance“ auslösen
1. Compare-Slider bedienen (Maus/Touch/Keyboard) und „Download“ prüfen
1. Quota-Anzeige/Toasts beobachten; bei Limit wird Button deaktiviert und Banner angezeigt
...
1. `CompareSlider` auslagern (DONE).
1. `Dropzone` auslagern (DONE).
1. `UsagePill` und `EnhancerActions` auslagern (DONE).
1. Obsolete States/Effects im Container entfernen (FOLLOW-UP).
