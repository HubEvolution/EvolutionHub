...
describe('Dashboard API Integration Tests', () => {
  it('GET /activity - sollte Aktivitäten mit gültiger Auth und DB-Daten zurückgeben', async () => {
    ...
    expect(mockLogUserEvent).toHaveBeenCalledWith(
      'user-123',
      'activity_feed_accessed',
      expect.any(Object)
    );
  });
  ...
  it('GET /notifications - sollte Notifications mit gültiger Auth und DB-Daten zurückgeben', async () => {
    ...
    expect(mockLogUserEvent).toHaveBeenCalledWith('user-123', 'notifications_viewed', {
      notificationCount: 1,
    });
  });
  ...
  it('GET /stats - sollte Stats mit Counts und Auth zurückgeben', async () => {
    mockDB
      .prepare()
      .first.mockResolvedValueOnce({ count: 3 }) // projects
    ...
    expect(mockDB.prepare).toHaveBeenNthCalledWith(
      1,
      expect.stringContaining('SELECT count(*) as count FROM projects')
    );
    expect(mockLogUserEvent).toHaveBeenCalledWith(
      'user-123',
