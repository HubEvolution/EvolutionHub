...
test.describe('Magic Link Flow (Stytch)', () => {
  ...
  test('should authenticate via dev-bypass callback and reach dashboard (local only)', async ({
    page,
  }) => {
    ...
    const session =
      (await getCookieValue(page, '__Host-session')) || (await getCookieValue(page, 'session_id'));
