...
const ENABLED =
  process.env.E2E_PROD_AUTH_SMOKE === '1' || process.env.E2E_PROD_AUTH_SMOKE === 'true';
...
test.describe('Prod Auth Smoke (hub-evolution.com)', () => {
  test.skip(
    !(ENABLED && isHubProd(BASE) && EMAIL),
    'Set E2E_PROD_AUTH_SMOKE=1, TEST_BASE_URL=https://hub-evolution.com and STYTCH_TEST_EMAIL to run'
  );

  test('POST /api/auth/magic/request returns success OR mapped provider error', async ({
    request,
  }) => {
  ...
  test('GET /login returns security headers (CSP, HSTS, X-Frame-Options, COOP)', async ({
    request,
  }) => {
    ...
    expect(
      (headers['strict-transport-security'] || headers['Strict-Transport-Security'])?.includes(
        'max-age'
      )
    ).toBeTruthy();
    expect((headers['x-frame-options'] || headers['X-Frame-Options'])?.toUpperCase()).toContain(
      'DENY'
    );
    expect(
      (
        headers['cross-origin-opener-policy'] || headers['Cross-Origin-Opener-Policy']
      )?.toLowerCase()
    ).toBe('same-origin');
  });

  test('GET /api/auth/callback without token redirects to login error', async ({ request }) => {
    const res = await request.get(new URL('/api/auth/callback', BASE).toString(), {
      maxRedirects: 0,
    });
    ...
    expect(
      location.includes('/en/login?magic_error=MissingToken') ||
        location.includes('/login?magic_error=MissingToken')
    ).toBeTruthy();
  ...
  test('GET /api/auth/callback with invalid token redirects to login InvalidOrExpired', async ({
    request,
  }) => {
    ...
    const location = String(loc);
    expect(
      location.includes('/en/login?magic_error=InvalidOrExpired') ||
        location.includes('/login?magic_error=InvalidOrExpired')
    ).toBeTruthy();
  });
});
